...
import { getTotalEarned } from '../core/web3';
import { getUserClaimTimes, getClaimPeriod, onClaimAstro } from '../core/Dashboard';
...
const Account = (props) => {
  ...
  const getInitAmount = useCallback(async () => {
    ...
    amount = Number(balance.astroBalance) * ROI_rate * nAstroPrice;
    setROIRateUSD(amount);

    setLoading(false);
  }, [web3, ROI_rate, wallet, balance.astroBalance, dailyRate, rebaseRate]);
  ...
  const handleClaim = async () => {
    ...
    let result = await onClaimAstro(claimAstro);
    if (result.success) {
          ...
          <Reveal keyframes={fadeInUp} className='full-card full-height onStep' delay={400} duration={1000} triggerOnce>
            <div className='full-height flex flex-column gap-5'>
              <div className='row'>
                <div className='col-md-6 pb-3'>
                  <div className='main-card'>
                    ...
                    <p className='card-value bold fs-40'>{loading ? <LoadingSkeleton /> : `$${numberWithCommas(dailyAstro * astroPrice)}`}</p>
                    <p className='card-value type-3'>{loading ? <LoadingSkeleton /> : `${numberWithCommas(dailyAstro)} ASTRO`}</p>
                  </div>
                </div>
                <div className='col-md-6 pb-3'>
                  <div className='main-card'>
              ...
              <div className='full-height flex justify-center align-items-center mb-4'>
                <a href={`https://dexscreener.com/avalanche/${config.avaxAstroPair}`} className='btn-main btn5 btn-activity' target='_blank' rel="noreferrer">DEX Charts</a>
                <button className='btn-main btn-activity' onClick={() => navigate('/swap')}>Buy ASTRO</button>
              </div>
            </div>
          </Reveal>
        </div>
            ...
            <div className='row'>
              <div className='col-md-5'>
                <div className='main-card'>
                  ...
                  <div className='flex flex-column'>
                    ...
                    <p className='fs-24 font-bold text-center mb-0'>{loading ? <LoadingSkeleton /> : '$' + numberWithCommas(nextRebaseAmount * astroPrice)}</p>
                    <p className='f-century-b fs-12 text-gray text-center'>{loading ? <LoadingSkeleton /> : numberWithCommas(nextRebaseAmount) + ' ASTRO'}</p>
                  </div>
                  <a className='text-center fs-14 hover:text-white cursor-pointer' data-tip data-for="main">ASTRO is auto-compounding</a>
                  <ReactToolTip className='tooltip' id="main" place={'top'} backgroundColor='#702ce9' textColor='white' type="dark" effect="solid" multiline={true}>
                    <span>ASTRO is autocompounding, so you do not need to claim unless you want to take profits. You simply hold and earn ASTRO automatically.</span>
                  </ReactToolTip>
                </div>
              </div>
              <div className='col-md-7'>
                <div className='claim-card full-height'>
                  ...
                  <p className='text-left fs-14 text-white'>If you choose to take your weekly claim, click below for 1% to maximize your growth.
                    If you'd like to take more earnings, <span className='underline cursor-pointer' onClick={() => navigate('/swap')}>swap here</span>. </p>
                  <div className='my-3 full-width'>
                    <div className='main-card new-card full-width flex flex-row justify-between align-items-center'>
                      <div className='flex align-items-center gap-2'>
                        <img src="/img/astro-icon.png" alt='' width="40px" height="40px"></img>
                        <p className='fs-16 text-white f-medium'>CLAIM ASTRO</p>
                      </div>
                      <p className='fs-28 astro-text'>{loading ? <LoadingSkeleton /> : numberWithCommas(claimAstro)}</p>
                    </div>
                  </div>
                  <div className='flex flex-column full-width'>
                    <div className='flex justify-between'>
                      <p className='fs-15 d-flex gap-1'>Your Earnings/Daily: <p className='fs-15 text-white'>1.91%</p></p>
                      <p className='fs-15 text-white'>{loading ? <LoadingSkeleton /> : `${numberWithCommas(Number(dailyAstro))} ($${numberWithCommas(Number(dailyAstro * astroPrice))})`}</p>
                    </div>
                    <div className='flex justify-between'>
                      <p className='fs-15 d-flex gap-1'>Recommended Claim: <p className='fs-15 text-white'>1%</p></p>
                      <p className='fs-15 text-white'>{loading ? <LoadingSkeleton /> : `${numberWithCommas(Number(balance.astroBalance) * 0.01)} ($${numberWithCommas(Number(balance.astroBalance * astroPrice) * 0.01)})`}</p>
                    </div>
                    <div className='flex justify-between'>
                      <p className='fs-15 d-flex gap-1'>Claim Tax: <p className='fs-15 text-white'>0%</p></p>
                      <p className='fs-15 text-white'>{loading ? <LoadingSkeleton /> : `${numberWithCommas(Number(balance.astroBalance) * 0)} ($${numberWithCommas(Number(balance.astroBalance * astroPrice) * 0)})`}</p>
                    </div>
                    <hr className='my-2' />
                  </div>
                  <div className='full-width flex justify-between align-items-center'>
                    ...
                    <span className='fs-26 text-green'>{loading ? <LoadingSkeleton /> : '$' + numberWithCommas(claimAstro * astroPrice)}</span>
                  </div>
                  <div className='full-width flex flex-column justify-center mt-2'>
                    {remainTime <= 0 ? (
                      <button className='d-flex btn-main fs-16 full-width justify-center' onClick={handleClaim}>Weekly Claim (1%)</button>
                    ) : (
                        ...
                        <div className='row'>
                          <div className='col-md-6 col-sm-12 my-2'>
                            <div className='d-flex flex-column'>
                              ...
                              <div className='flex justify-between'>
                                <p className='fs-12'>Buy / Transfer Tax</p>
                                <p className='fs-12 text-white'>15%</p>
                              </div>
                              <div className='flex justify-between'>
                                <p className='fs-12'>Sell Tax</p>
                                <p className='fs-12 text-white'>30%</p>
                              </div>
                            </div>
                          </div>
                          <div className='col-md-6 col-sm-12 tax-guide-left my-2'>
                            <div className='d-flex flex-column'>
                              ...
                              <div className='flex justify-between'>
                                <p className='fs-12'>USD {`>`} $25,001</p>
                                <p className='fs-12 text-white'>80%</p>
                              </div>
                              <div className='flex justify-between'>
                                <p className='fs-12'>USD $10,001 - $25,000</p>
              ...
              <div className='col-md-12'>
                <div className="flex flex-column px-3" style={{ gap: '14px' }}>
                  <div className='flex flex-column flex-md-row justify-between'>
                    <label className='claim-label'>Current ASTRO Price</label>
                    <p className='f-medium mb-0 text-right'>{loading ? <LoadingSkeleton /> : '$ ' + numberWithCommas(astroPrice, 4) + ' USD'}</p>
                  </div>
                  <div className='flex flex-column flex-md-row justify-between'>
                    <label className='claim-label'>Next Reward Amount</label>
                    <p className='f-medium mb-0 text-yellow text-right'>{loading ? <LoadingSkeleton /> : numberWithCommas(nextRebaseAmount) + ' ASTRO'}</p>
                  </div>
                  <div className='flex flex-column flex-md-row justify-between'>
                    <label className='claim-label'>Next Reward Amount USDC</label>
                    <p className='f-medium mb-0 text-right'>{loading || astroPrice === '' ? <LoadingSkeleton /> : '$ ' + numberWithCommas(nextRebaseAmount * astroPrice) + ' USD'}</p>
                  </div>
                  <div className='flex flex-column flex-md-row justify-between'>
