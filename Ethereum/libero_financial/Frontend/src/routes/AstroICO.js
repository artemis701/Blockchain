...
const AstroICO = (props) => {
  ...
  const handleSelectCoin = async (value) => {
    ...
    if (value === 0) {
      ...
      if (result.success) {
        ...
        setPAstroAmount(Number(result.value) / usdcPrice);
      }
    } else {
      setPAstroAmount(fromToken / usdcPrice);
    }
  }
  ...
  const handleChange = async (event) => {
    ...
    if (coinType === 0) {
      ...
      setPAstroAmount(value / usdcPrice);
    }
  }
  ...
  const validate = () => {
    ...
    if ((coinType === 0 && Number(balance.avaxBalance) < Number(tokenAmountA)) || (coinType === 1 && Number(balance.usdcBalance) < Number(tokenAmountA))) {
      toast.error("You have insufficient amount to buy ASTRO.");
      return false;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        Swal.fire({
          icon: 'success',
          title: ' Success',
          text: 'You have bought ASTRO for presale successfully.'
        });
      } else {
    ...
    <div className='page-container text-center ico-container'>
      ...
      <div className='ico-header'>
        <Reveal className='onStep' keyframes={fadeInUp} delay={0} duration={600} triggerOnce>
          <p className='ico-title'>Welcome to the ASTRO Presale</p>
        </Reveal>
        <Reveal className='onStep' keyframes={fadeInUp} delay={300} duration={600} triggerOnce>
          <p className="ico-desc">
            Presale term - April 15th - April 20th 12:00 UTC <br />( 5 Days or until we reach our goal of 1.6 Million USDC )<br />
            Powered by AVAX. You can participate using AVAX or USDC.
          </p>
        </Reveal>
        <Reveal className='onStep' keyframes={fadeInUp} delay={600} duration={600} triggerOnce>
          <p className="ico-desc">
            With a presale price of 0.008 USDC.<br />
            Our minimum limit will be $100 USDC and a max of $4,000 USDC.<br />
              ...
              <div className='presale-inner pt-3 pb-4'>
                <div className="row">
                    ...
                    <>
                      {getUTCNow() / 1000 >= endTime || leftCap <= 0 ? (
                        <>
                          ...
                          <p className='fs-20 fw-bold mt-3 mb-1 text-gray'>Your Holdings (ASTRO): <span className='text-white'>{paidUSDC === '' ? <LoadingSkeleton /> : numberWithCommas(paidUSDC / Number(usdcPrice))}</span></p>
                        </>
                      ) : (
                        <>
                  ...
                  <div className='flex flex-column mt-3'>
                    ...
                    <p className='fs-20 mt-2'>Please swap your ASTRO to ASTRO.</p>
                  </div>
                </div>
              </div>
            </section>
              ...
              <div className='presale-inner'>
                <div className="row justify-center">
                  ...
                  <div className='col-md-12 mt-3'>
                    {isMobile ? (
                      <>
                        <p className='fs-20 mb-1'>Presale Amount received <br /><strong>{curPresale === '' ? <LoadingSkeleton /> : '$' + numberWithCommas(curPresale) + ' USDC'}</strong></p>
                        <p className='fs-20 mb-1'>Maximum Presale Amount Allocated <br /><strong>{usdcPrice === '' ? <LoadingSkeleton /> : '$' + numberWithCommas(max_token_amount * Number(usdcPrice)) + ' USDC'}</strong></p>
                        <p className='fs-20 mb-1'>ASTRO Price <br /><strong>{usdcPrice === '' ? <LoadingSkeleton /> : '$' + numberWithCommas(Number(usdcPrice)) + ' USDC'}</strong></p>
                      </>
                    ) : (
                      <>
                        <p className='fs-20 mb-1'>Presale Amount received : <strong>{curPresale === '' ? <LoadingSkeleton /> : '$' + numberWithCommas(curPresale) + ' USDC'}</strong></p>
                        <p className='fs-20 mb-1'>Maximum Presale Amount Allocated : <strong>{usdcPrice === '' ? <LoadingSkeleton /> : '$' + numberWithCommas(max_token_amount * Number(usdcPrice)) + ' USDC'}</strong></p>
                        <p className='fs-20 mb-1'>ASTRO Price : <strong>{usdcPrice === '' ? <LoadingSkeleton /> : '$' + numberWithCommas(Number(usdcPrice)) + ' USDC'}</strong></p>
                      </>
                    )}
                  </div>
                  {(/*startPresale && */leftCap > 0) && (
                    <>
                      {maxTotalCap <= paidUSDC ? (
                        <div className='buy_content'>
                          <p className='fs-20 mb-1'>You have got the maximum ASTRO on presale.</p>
                          <p className='fs-20 mb-1'>Your Holdings (ASTRO): {paidUSDC === '' ? <LoadingSkeleton /> : numberWithCommas(paidUSDC / Number(usdcPrice))}</p>
                        </div>
                      ) : (
                        <div className='col-md-12 mt-3'>
                          <div className='buy_content'>
                            <div className='row'>
                              <div className='col-md-12'>
                                <p className='fs-20'>Please enter the ASTRO amount you'd like to purchase</p>
                                    ...
                                    <div className="d-flex justify-content-between input-box">
                                      ...
                                      <div className='flex align-items-center gap-2' style={{ padding: '10px' }}>
                                        <img
                                          ...
                                          alt={`Coin of ASTRO`}
                                        />
                                        <span className='fs-20'>{'ASTRO'}</span>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                              ...
                              <div className='col-md-12 mt-3'>
                                <p className='fs-20'>ASTRO remaining for your wallet limit: {maxCap === '' || usdcPrice === '' ? <LoadingSkeleton /> : numberWithCommas(maxCap / Number(usdcPrice))}
                                  {coinType === 0 ? ` (${numberWithCommas(maxAvaxCap)} AVAX)` : ` ($${numberWithCommas(maxCap)} USDC)`}</p>
                                <p className='fs-20'>Minimum per transaction is $100, Maximum for presale is $4,000.</p>
                                <LoadingButton
                                  onClick={handleBuy}
                                  endIcon={<></>}
                                  loading={pending}
                              ...
                              <div className='flex justify-center align-items-center gap-3 mt-3 cursor-pointer' onClick={addTokenCallback}>
                                ...
                                <span style={{ whiteSpace: 'nowrap' }}> Add ASTRO to MetaMask</span>
                              </div>
                            </div>
                          </div>
                        </div>
                    ...
                    //       <p className='fs-20 mb-1'>Limit pet Wallet: <strong>1000 ASTRO</strong></p>
                    //       <p className='fs-20 mb-1'>Starting ASTRO Price: <strong>0.008 USDC</strong></p>
