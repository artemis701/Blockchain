...
function AstroChart() {
  ...
  useEffect(() => {
    ...
    let newData = [];
    if (data.length > 1) {
      newData = data.concat({ time: moment(new Date((new Date()).getTime() + updateInterval * 3600 * 24 * 1000)).format("YYYY-MM-DD"), value: astroPrice });
    } else {
      newData = data.concat({ time: moment(new Date()).format("YYYY-MM-DD"), value: astroPrice });
    }
  ...
  return (
    <div className='row full-height'>
      <div className='col-md-12 mt-2 mb-2 flex flex-column flex-md-row justify-between align-items-center'>
        <p className='fs-16 f-semi-b text-white mb-0 lh-1'>${numberWithCommas(astroPrice, 5)} / ASTRO</p>
